...
verify_mem_pool() {
	MEMPOOL=$(container-cli conway query tx-mempool --testnet-magic 4 tx-exists $(container-cli conway transaction txid --tx-file ./txs/hornan/action-tx.signed) | jq .exists)
	while true; do
...
sleep 0.2
        ...
        --witness-override 2 \
        --out-file ./txs/hornan/action-tx.raw
        
# Remove the action index options file        
rm -rf ./txs/hornan/action-create 2>/dev/null
...
sleep 0.2
        ...
        container-cli transaction sign --tx-body-file ./txs/hornan/action-tx.raw \
        --signing-key-file ./keys/stake.skey \
        --signing-key-file ./keys/payment.skey \
        --testnet-magic 4 \
        --out-file ./txs/hornan/action-tx.signed
...
sleep 0.2
        ...
        --testnet-magic 4 \
        --tx-file ./txs/hornan/action-tx.signed > ./txs/hornan/my_outputs.txt 2>&1
 	...
 	# Add the governance action to a sharable list
  	if [ "$(cat ./txs/hornan/my_outputs.txt)" == "Transaction successfully submitted." ]; then
   	  ...
   	  --tx-file ./txs/hornan/action-tx.signed >> ./txs/hornan/actionsID.txt
