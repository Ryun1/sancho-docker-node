...
# ~~~~~~~~~~~~ CHANGE THIS ~~~~~~~~~~~~
DEPOSIT_RETURN_STAKE_ADDR="stake_test1uz6ljatyc7w52z44hskd5pu5cvw7qemwz6re3ux4pmdqumcn2qyrx"
...
container_cli conway governance action create-info \
  ...
  --deposit-return-stake-address "$DEPOSIT_RETURN_STAKE_ADDR" \
...
container_cli conway transaction build \
 ...
 --proposal-script-file ./keys/multi-sig/script.json \
...
# Create orchestrator witness
container_cli conway transaction witness \
  --testnet-magic 4 \
  --tx-body-file ./txs/multi-sig/info-action-tx.unsigned \
  --signing-key-file ./keys/payment.skey \
  --out-file ./keys/multi-sig/orchestrator-payment.witness

# Assemble Transaction
container_cli transaction assemble \
  --tx-body-file ./txs/multi-sig/info-action-tx.unsigned \
  --witness-file ./keys/multi-sig/orchestrator-payment.witness \
